...
class LidarBicycleCircle(LidarCircle):
    ...
    def __init__(
        ...
        if self.params['n_move_obs'] % 2 == 1:
            self.params['n_move_obs'] += 1
    ...
    def node_dim(self) -> int:
        return 10  # state dim (4) + indicator: agent 00001, goal 00010, obstacle 00100, move_obstacle 01000, fast move_obstacle 10000
    ...
    def reset(self, key: Array) -> GraphsTuple:
        ...
        rand_key, key = jr.split(key)
        rand_var = jr.uniform(rand_key, (self.num_agents, 1), minval=0, maxval=1)
        
        theta_key, key = jr.split(key)
        thetas = jnp.linspace(0, 2 * jnp.pi, self.num_agents + 1)[:-1]
        thetas += jr.uniform(theta_key, (1,), minval=0, maxval=2 * jnp.pi)
        agent_pos = jnp.stack([R * jnp.cos(thetas), R * jnp.sin(thetas)], axis=-1) + self.area_size / 2
        # states = states.at[:, :2].set(agent_pos)
        # states = jnp.where(rand_var < 0.5, states.at[:, :2].set(agent_pos), states)
        states = states.at[:, :2].set(jnp.where((rand_var < 0.2).repeat(2,axis=-1), agent_pos, states[:, :2]))
        ...
        if self.params['n_move_obs'] == 0:
            ...
            obs_vel = obs_vel.at[-self.params['n_move_obs'] // 2:].set(self._params['move_obs_vel'] * 2)
    ...
    def step(
        ...
        obs_thetas_next = obs_thetas_next.at[-self.params['n_move_obs'] // 2:].set(
            obs_thetas_next[-self.params['n_move_obs'] // 2:] + self._params['move_obs_vel'] * self.dt / (self._params['R'] * 2 / 3))
        ...
        next_obs_vel = next_obs_vel.at[-self.params['n_move_obs'] // 2:].set(self._params['move_obs_vel'] * 2)
    ...
    def goal_reach(self, graph: GraphsTuple) -> Array:
        agent = graph.env_states.agent
        goal = graph.env_states.goal
        dist = jnp.linalg.norm(agent[:, :2] - goal[:, :2], axis=-1)
        return dist < self._params['dist2goal']
