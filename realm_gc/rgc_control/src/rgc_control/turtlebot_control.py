...
class TurtlebotControl(RobotControl):
    def __init__(self):
        ...
        self.control = TurtlebotAction(0.0,0.0)#self.control = np.zeros((2,1))
        ...
        #self.control_policy = TrajectoryTrackingPolicy(self.trajectory,TurtlebotSteeringPolicy) 
        self.control_policy = create_tro_turtlebot_policy(np.zeros((2,1)),self.eqx_filepath)
    ...
    def update(self):
        ...
        if self.state is not None:
            ...
            #Pack [x,y,theta,v] from state message and control policy into TimedPose2DObservation instance. 
            v = self.control.linear_velocity
            t = rospy.Time.now()-self.time_begin
            current_state = TimedPose2DObservation(self.state.x,self.state.y,self.state.theta,v,t)
