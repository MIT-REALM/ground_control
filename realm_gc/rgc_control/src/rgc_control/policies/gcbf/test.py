...
def test(args):
    ...
    env = make_env(
        ...
        num_obs=args.obs,
        num_mov_obs=args.mov_obs,
        mov_obs_speed=args.mov_obs_speed if not 'mov_obs_speed' in config else max(config.mov_obs_speed,args.mov_obs_speed),
    ...
    if args.nojit_rollout:
        ...
        print("jit rollout!")
        if args.use_qp:
            # rollout_fn = jax_jit_np(env.rollout_qp_fn(qp_fn, args.max_step))
            act_fn = qp_fn
            rollout_fn = jax_jit_np(env.rollout_qp_fn(act_fn, args.max_step))
            print('using qp rollout')
        else:
            rollout_fn = jax_jit_np(env.rollout_fn(act_fn, args.max_step))
            print('using learned policy rollout')

        is_unsafe_fn = jax_jit_np(jax_vmap(env.collision_mask))
        is_finish_fn = jax_jit_np(jax_vmap(env.finish_mask))
        agent_coll_fn = jax_jit_np(jax.vmap(env.agent_collision_mask))
        obs_coll_fn = jax_jit_np(jax.vmap(env.obs_collision_mask))
        mov_obs_coll_fn = jax_jit_np(jax.vmap(env.mov_obs_collision_mask))
    ...
    for i_epi in range(args.epi):
        ...
        if args.nojit_rollout:
            ...
            agent_colls.append(agent_coll_fn(rollout.Tp1_graph))
            obs_colls.append(obs_coll_fn(rollout.Tp1_graph))
            mov_obs_colls.append(mov_obs_coll_fn(rollout.Tp1_graph))
        ...
        agent_coll_rate = agent_colls[-1].max(axis=0).mean()
        obs_coll_rate = obs_colls[-1].max(axis=0).mean()
        mov_obs_coll_rate = mov_obs_colls[-1].max(axis=0).mean()
        ...
        print(f"epi: {i_epi}, reward: {epi_reward:.3f}, cost: {epi_cost:.3f}, "
                ...
                f"agent coll rate: {agent_coll_rate * 100:.3f}%, "
                f"obs coll rate: {obs_coll_rate * 100:.3f}%, "
                f"mov obs coll rate: {mov_obs_coll_rate * 100:.3f}%")
    ...
    agent_colls = np.max(np.stack(agent_colls), axis=1)
    obs_colls = np.max(np.stack(obs_colls), axis=1)
    mov_obs_colls = np.max(np.stack(mov_obs_colls), axis=1)
    
    agent_col_mean, agent_col_std = agent_colls.mean(), agent_colls.std()
    obs_col_mean, obs_col_std = obs_colls.mean(), obs_colls.std()
    mov_obs_col_mean, mov_obs_col_std = mov_obs_colls.mean(), mov_obs_colls.std()
    ...
    print(
            ...
            f"agent_coll_rate: {agent_col_mean * 100:.3f}%, "
            f"obs_coll_rate: {obs_col_mean * 100:.3f}%, "
            f"mov_obs_coll_rate: {mov_obs_col_mean * 100:.3f}%"
...
def main():
    ...
    parser.add_argument("--mov_obs_speed", type=float, default=0.1)
    parser.add_argument("--mov-obs", type=int, default=4)
