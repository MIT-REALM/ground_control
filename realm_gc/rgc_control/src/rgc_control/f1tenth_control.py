...
from rgc_control.policies.tracking.tracking_policies import TrajectoryTrackingPolicy, TimedPose2DObservation
...
class F1TenthControl(RobotControl):
    def __init__(self):
        ...
        self.control_pub = rospy.Publisher('/vesc/high_level/ackermann_cmd_mux/input/nav_0',F1TenthDriveStamped,queue_size=1) 
        self.control = F1TenthAction(0.0,0.0)

        #Subscribe to state estimation topic from ros param
        self.state = None
        self.state_estimate_topic = rospy.get_param("~state_estimate_topic", f"{rospy.get_name()}/estimate")
        ...
        self.mlp_eqx = rospy.get_param('~mlp/base_path')+rospy.get_param('~mlp/filename')
        
        #Instantiate control policy using Turtlebot steering policy and reference trajectory 
        #self.control_policy = TrajectoryTrackingPolicy(self.trajectory,F1TenthSteeringPolicy) 
        self.control_policy = create_tro_f1tenth_policy(np.zeros((4,1)),self.eqx_filepath,self.mlp_eqx)
    ...
    def update(self):
        ...
        if self.state is not None:
            ...
            current_state = TimedPose2DObservation(self.state.x,self.state.y,self.state.theta,self.state.speed,t)
            self.control = self.control_policy.compute_action(current_state)

        msg = F1TenthDriveStamped() #Not sure if this is the right message template
