...
def get_node_goal_rng(
    ...
    def non_valid_node(reset_input: Tuple[int, Array, Array, Array]):  # key, node, all nodes
        ...
        if mov_obs is None:
            collide_mov_obs = False
        else:
            dist_mov_obs = jnp.linalg.norm(mov_obs[:, :2] - node, axis=1).min()
            collide_mov_obs = dist_mov_obs <= min_dist
        inside = inside_obstacles(node, obstacles, r=min_dist)
        valid = ~(collide | inside | collide_mov_obs) | (i_iter >= max_iter)
        return ~valid
    ...
    def non_valid_goal(reset_input: Tuple[int, Array, Array, Array, Array]):
        ...
        if mov_obs is None:
            ...
            dist_mov_obs = jnp.linalg.norm(mov_obs[:, :2] - goal, axis=1).min()
            collide_mov_obs = dist_mov_obs <= min_dist
        ...
        valid = (~collide & ~inside & ~outside & ~too_long & ~collide_mov_obs) | (i_iter >= max_iter)
        out = ~valid
