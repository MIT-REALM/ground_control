...
class SingleIntegrator(MultiAgentEnv):
    ...
    def node_dim(self) -> int:
        return 4  # indicator: agent: 0001, goal: 0010, stationary obstacle: 0100, moving obstacle: 1000
    ...
    def reset(self, key: Array) -> GraphsTuple:
        ...
        mov_obs_vel = jr.uniform(mov_obs_vel_key, (n_rng_mov_obs, 2), minval=-self._params["mov_obs_speed"], maxval=self._params["mov_obs_speed"])
        self.mov_obs_vel = mov_obs_vel
    ...
    def get_cost(self, graph: GraphsTuple) -> Cost:
        ...
        # collision = inside_obstacles(agent_pos, mov_obstacles, r=2 * self._params["car_radius"])
        collision_mov_obs = jnp.linalg.norm(jnp.expand_dims(agent_pos, 1) - jnp.expand_dims(mov_obstacles, 0), axis=-1)
        collision = (2 * self._params["car_radius"] > collision_mov_obs).any(axis=1)
        cost += collision.mean()
    ...
    def get_graph(self, state: EnvState) -> GraphsTuple:
        ...
        node_feats = jnp.zeros((self.num_agents * 2 + n_hits + num_mov_obs, 4))
        node_feats = node_feats.at[: self.num_agents, 3].set(1)  # agent feats
        node_feats = node_feats.at[self.num_agents: self.num_agents * 2, 2].set(1)  # goal feats
        node_feats = node_feats.at[-n_hits-num_mov_obs:-num_mov_obs, 1].set(1)  # obs feats
        node_feats = node_feats.at[-num_mov_obs:, 0].set(1)  # mov obs feats

        # node type
        # node_type = jnp.zeros(n_nodes, dtype=jnp.int32)
        node_type = -jnp.ones(n_nodes, dtype=jnp.int32)
        node_type = node_type.at[: self.num_agents].set(SingleIntegrator.AGENT)
        node_type = node_type.at[self.num_agents: self.num_agents * 2].set(SingleIntegrator.GOAL)
        node_type = node_type.at[-n_hits-self._params["n_mov_obs"]: -self.params["n_mov_obs"]].set(SingleIntegrator.OBS)
        node_type = node_type.at[-self._params["n_mov_obs"]:].set(SingleIntegrator.MOV_OBS)
    ...
    def safe_mask(self, graph: GraphsTuple) -> Array:
        ...
        safe_mov_obs = jnp.greater(mov_obs_dist, self._params["car_radius"] * 4.5)    
