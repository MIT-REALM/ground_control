...
class DubinsCar(MultiAgentEnv):
    ...
    def get_graph(self, state: EnvState, adjacency: Array = None) -> GraphsTuple:
        ...
        node_feats = node_feats.at[-n_hits-num_mov_obs:-num_mov_obs, 0].set(1)  # obs feats
        node_feats = node_feats.at[-num_mov_obs:-num_mov_obs//2, 2].set(1)  # slow mov obs feats
        node_feats = node_feats.at[-num_mov_obs//2:, 2].set(1)  # fast mov obs feats
        ...
        node_type = node_type.at[: self.num_agents].set(DubinsCar.AGENT)
        node_type = node_type.at[self.num_agents: self.num_agents * 2].set(DubinsCar.GOAL)
        node_type = node_type.at[-n_hits-self._params["n_mov_obs"]: -self.params["n_mov_obs"]].set(DubinsCar.OBS)
        node_type = node_type.at[-self._params["n_mov_obs"]:-self._params["n_mov_obs"] // 2].set(DubinsCar.SLOW_MOV_OBS)
        node_type = node_type.at[-self._params["n_mov_obs"] // 2:].set(DubinsCar.FAST_MOV_OBS)
    ...
    def collision_mask(self, graph: GraphsTuple) -> Array:
        ...
        slow_mov_obs_dist = agent_pos[:, None, :2] - mov_obs_pos[None, :self._params["n_mov_obs"] // 2, :]
        slow_mov_obs_dist = jnp.linalg.norm(slow_mov_obs_dist, axis=-1)
        unsafe_slow_mov_obs = jnp.less(slow_mov_obs_dist, self._params["car_radius"]).any(axis=1)
        
        # agents are colliding with fast moving obstacles
        fast_mov_obs_dist = agent_pos[:, None, :2] - mov_obs_pos[None, self._params["n_mov_obs"] // 2:, :]
        fast_mov_obs_dist = jnp.linalg.norm(fast_mov_obs_dist, axis=-1)
        unsafe_fast_mov_obs = jnp.less(fast_mov_obs_dist, self._params["car_radius"]).any(axis=1)
