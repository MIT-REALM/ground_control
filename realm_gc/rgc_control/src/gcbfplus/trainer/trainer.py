...
class Trainer:
    ...
    def __init__(
        ...
        wandb.login()
        if env_name == 'SingleIntegratorAdapt':
            env_initials = 'SIA'
        elif env_name == 'SingleIntegratorAdaptEdge':
            env_initials = 'SIAE'
        elif env_name == 'DoubleIntegratorAdapt':
            env_initials = 'DIA'
        elif env_name == 'SingleIntegrator':
            env_initials = 'SI'
        elif env_name == 'DoubleIntegrator':
            env_initials = 'DI'
        elif env_name == 'DubinsCar':
            env_initials = 'DC'
        elif env_name == 'DubinsCarAdapt':
            env_initials = 'DCA'
        else:
            env_initials = 'Unknown'
        wandb.init(name=params['run_name'], project='gcbf-nf5-' + env_initials, dir=self.log_dir)
    ...
    def train(self):
        ...
        collision_fun = jax_vmap(jax_vmap(self.env_test.collision_mask))
        agent_coll_fn = jax_vmap(jax_vmap(self.env_test.agent_collision_mask))
        obs_coll_fn = jax_vmap(jax_vmap(self.env_test.obs_collision_mask))
        mov_obs_coll_fn = jax_vmap(jax_vmap(self.env_test.mov_obs_collision_mask))
        ...
        for step in range(0, self.steps + 1):
            ...
            if step % self.eval_interval == 0:
                ...
                collision = collision_fun(test_rollouts.graph).max(axis=1).mean()
                agent_collision = agent_coll_fn(test_rollouts.graph).max(axis=1).mean()
                obs_collision = obs_coll_fn(test_rollouts.graph).max(axis=1).mean()
                mov_obs_collision = mov_obs_coll_fn(test_rollouts.graph).max(axis=1).mean()
                ...
                eval_verbose = (f'step: {step:3}, time: {time_since_start:5.0f}s, reward: {reward_mean:9.4f}, '
                                ...
                                f'unsafe_frac: {unsafe_frac:6.2f}, finish: {finish:6.2f}, collision: {collision:6.2f}, '
                                f'agent_collision: {agent_collision:6.2f}, obs_collision: {obs_collision:6.2f}, '
                                f'mov_obs_collision: {mov_obs_collision:6.2f}')
