...
class GCBFPlus(GCBF):
    ...
    def update_nets(self, rollout, safe_mask, unsafe_mask):
        ...
        prev_graph = jtu.tree_map(lambda x: x[:-1], rollout.graph)
        graph0 = jtu.tree_map(lambda x: x[0], rollout.graph)
        prev_graph = tree_merge([jtu.tree_map(lambda x: x[None, ...], graph0), prev_graph])

        # t0 = time.time()
        b_u_qp = []
        for ii in range(n_chunks):
            graph = jtu.tree_map(lambda x: x[ii * chunk_size: (ii + 1) * chunk_size], rollout.graph)
            prev_graph_chunk = jtu.tree_map(lambda x: x[ii * chunk_size: (ii + 1) * chunk_size], prev_graph)
            # b_u_qp.append(jax2np(self.get_b_u_qp(graph, self.cbf_tgt.params)))
            b_u_qp.append(jax2np(jax.vmap(ft.partial(self.get_u_qp_act, params=self.cbf_tgt.params))(graph, prev_graph_chunk)))
