...
from .utils import rollout, test_rollout
...
class Trainer:
    ...
    def train(self):
        ...
        #     return rollout(self.env,
        #                    ft.partial(self.algo.step, params=params),
        #                    ft.partial(self.algo.get_value, params=params) if hasattr(self.algo, 'get_value') else None,
        #                    init_rnn_state,
        #                    init_rnn_state,
        #                    key,
        #                    self.gamma)
        ...
        zmax_fn = lambda graph, params: jnp.array([[-self.env_test.reward_min]]).repeat(self.env.num_agents, axis=0)
        zmin_fn = lambda graph, params: jnp.array([[-self.env_test.reward_max]]).repeat(self.env.num_agents, axis=0)
        ...
        slow_mov_coll_fn = jax_vmap(jax_vmap(self.env_test.slow_mov_obs_collision_mask))
        fast_mov_coll_fn = jax_vmap(jax_vmap(self.env_test.fast_mov_obs_collision_mask))
        reach_mask = jax_vmap(jax_vmap(self.env_test.goal_mask))
        ...
        for step in range(0, self.steps + 1):
            ...
            if step % self.eval_interval == 0:
                ...
                if step % self.full_eval_interval == 0:
                    ...
                    slow_mov_obs_collision = slow_mov_coll_fn(test_rollouts.graph).max(axis=1).max(axis=-1).mean()
                    fast_mov_obs_collision = fast_mov_coll_fn(test_rollouts.graph).max(axis=1).max(axis=-1).mean()
                    reach = reach_mask(test_rollouts.graph).max(axis=1).mean()
                    # breakpoint()
                    eval_info = eval_info | {
                    ...
                    eval_verbose = (f'step: {step:3}, time: {time_since_start:5.0f}s, reward: {reward_mean:9.4f}, '
                                    ...
                                    f'mov_obs_collision: {slow_mov_obs_collision:6.2f}, '
                                    f'fast_mov_obs_collision: {fast_mov_obs_collision:6.2f}, '
                                    f'reach rate: {reach:6.2f}')
            ...
            # rollouts: Rollout = rollout_fn(self.algo.params, key_x0)
