...
def get_node_goal_rng(
    ...
    slow_mov_obs_dist = r * 2
    fast_mov_obs_dist = r * 4
    num_obs = mov_obs.shape[0]
    min_dist = 2.2 * r
    ...
    def non_valid_node(reset_input: Tuple[int, Array, Array, Array]):  # key, node, all nodes
        ...
        dist_obs = jnp.linalg.norm(mov_obs - node, axis=1)
        slow_obs_dist = dist_obs[:num_obs//2].min()
        fast_obs_dist = dist_obs[num_obs//2:].min()
        collide_slow_obs = slow_obs_dist <= slow_mov_obs_dist 
        collide_fast_obs = fast_obs_dist <= fast_mov_obs_dist
        inside = inside_obstacles(node, obstacles, r=min_dist / 2)
        valid = ~(collide | inside | collide_slow_obs | collide_fast_obs) | (i_iter >= max_iter)
        return ~valid
    ...
    def non_valid_goal(reset_input: Tuple[int, Array, Array, Array, Array]):
        ...
        collide = dist_min <= min_dist
        dist_obs = jnp.linalg.norm(mov_obs - goal, axis=1)
        slow_obs_dist = dist_obs[:num_obs//2].min()
        fast_obs_dist = dist_obs[num_obs//2:].min()
        collide_slow_obs = slow_obs_dist <= slow_mov_obs_dist 
        collide_fast_obs = fast_obs_dist <= fast_mov_obs_dist
        ...
        valid = (~collide & ~inside & ~outside & ~too_long & ~collide_slow_obs & ~collide_fast_obs) | (i_iter >= max_iter)
        out = ~valid
