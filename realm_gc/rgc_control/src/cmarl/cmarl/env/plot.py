...
def render_mpe(
    ...
    if dim == 1:
        # alphas = [0. if e_is_goal[ii] else 0.5 for ii in range(len(e_start))]
    ...
    if dim == 1 or dim == 2:
        ...
        z_text = ax.text2D(0.5, 1.04, "z: []", va="bottom", ha="center", **text_font_opts)

    # text for goal distance
    # if dim == 1 or dim == 2:
    #     goal_dist_text = ax.text(0.99, 1.00, "Goal distance: []", va="bottom", ha="right", **text_font_opts)
    # else:
    #     goal_dist_text = ax.text2D(0.99, 1.00, "Goal distance: []", va="bottom", ha="right", **text_font_opts)

    # text for the action
    # if dim == 1 or dim == 2:
    #     action_text = ax.text(0.5, 1.04, "Action: []", va="bottom", ha="center", **text_font_opts)
    # else:
    #     action_text = ax.text2D(0.5, 1.04, "Action: []", va="bottom", ha="center", **text_font_opts)
    ...
    def update(kk: int) -> list[plt.Artist]:
        ...
        # update the goal distance text
        # agent_pos = n_pos_t[:n_agent]
        # goal_pos = n_pos_t[n_agent:n_agent * 2]
        # dist2goal = np.linalg.norm(jnp.expand_dims(goal_pos, 1) - jnp.expand_dims(agent_pos, 0), axis=-1).min(axis=1)
        # np.set_printoptions(precision=4)
        # goal_dist_text.set_text(f"Goal dist: {dist2goal}")

        # update the action text
        # if kk < len(rollout.actions):
        #     action_text.set_text(f"Action: {rollout.actions[kk]}")
