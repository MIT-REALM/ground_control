...
class EFInforMARL(Algorithm):
    def __init__(
        ...
        super(EFInforMARL, self).__init__(
            env=env,
            node_dim=node_dim,
            edge_dim=edge_dim,
    ...
    def update(self, rollout: Rollout, step: int) -> dict:
        ...
        hyper_params = HyperParams(coef_ent=self.coef_ent)

        # hyper_params = {"coef_ent": self.coef_ent}
        if self.coef_ent_schedule and step < 1e5:
            # hyper_params["coef_ent"] = self.coef_ent / 1e5 * (1e5 - step)
            hyper_params = hyper_params._replace(coef_ent=self.coef_ent / 1e5 * (1e5 - step))
    ...
    def update_inner(
        ...
        # bTp1h_Vh = bTp1ah_Vh.max(-2)
        # bTp1h_Vh = bTp1ah_Vh.reshape(bTp1ah_Vh.shape[0], bTp1ah_Vh.shape[1], -1)

        # # calculate EFOCP GAE
        # bTh_Qh, bT_Ql, bT_Q = jax.vmap(
        #     ft.partial(compute_efocp_gae, disc_gamma=self.gamma, gae_lambda=self.gae_lambda)
        # )(Th_hs=rollout.costs.reshape(bTp1ah_Vh.shape[0], bTp1ah_Vh.shape[1] - 1, -1),
        #   T_l=-rollout.rewards,
        ...
        # bT_V = jax_vmap(jax_vmap(compute_efocp_V))(rollout.zs.squeeze(-1).sum(-1), bTah_Vh.max(-2), bTa_Vl.sum(-1))
        # bT_A = bT_Q - bT_V
        # # bT_A = bT_Ql - bTa_Vl.sum(-1)
        # bT_A = (bT_A - bT_A.mean(axis=1, keepdims=True)) / (bT_A.std(axis=1, keepdims=True) + 1e-8)
        ...
        def update_fn(carry, idx):
            ...
            policy, policy_info = self.update_policy(policy, rollout_batch, bTa_A[idx], rnn_chunk_ids, hyper_params)
    ...
    def update_policy(
        ...
        def get_loss(params):
            ...
            policy_loss = loss_policy - hyper_params.coef_ent * mean_entropy
