...
from .utils import compute_dec_efocp_gae, HLGaussTransform
...
class InforMARLLagr(InforMARL):
    ...
    def __init__(
        ...
        self.Vl_sigma = hl_gauss_smooth_ratio * (env.reward_max - env.reward_min) / hl_gauss_bins
        self.Vl_hl_gauss_trans = HLGaussTransform(
            -self._env.reward_max,
            -self._env.reward_min,
            hl_gauss_bins,
            self.Vl_sigma
        )

        self.Vh_sigma = hl_gauss_smooth_ratio * (env.cost_max - env.cost_min) / hl_gauss_bins
        self.Vh_hl_gauss_trans = HLGaussTransform(
            self._env.cost_min,
            self._env.cost_max,
            hl_gauss_bins,
            self.Vh_sigma
        ...
        self.Vh = CostValueNet(
            ...
            hl_gauss_bins=hl_gauss_bins if self.use_hl_gauss else None,
        )
        ...
        Vh_params = self.Vh.net.init(Vh_key, self.nominal_graph, self.init_Vh_rnn_state, self.n_agents, self.nominal_z)
    ...
    def update_Vh(
        ...
        def get_loss_hl_gauss_(params):
            ...
            bcTah_Qh_dist = rep_vmap(self.Vh_hl_gauss_trans.value2dist, rep=5)(bcTah_Qh)
            loss_Vh = optax.softmax_cross_entropy(bcTah_Vh, bcTah_Qh_dist).mean()
            return loss_Vh

        if self.use_hl_gauss:
            loss, grad = jax.value_and_grad(get_loss_hl_gauss_)(Vh_train_state.params)
        else:
