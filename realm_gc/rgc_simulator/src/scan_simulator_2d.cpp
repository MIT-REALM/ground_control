...
#include "rgc_simulator/distance_transform.hpp"
...
const std::vector<double> ScanSimulator2D::scan(const rgc_simulator::Pose2d & pose) {
  scan(pose, scan_output.data());
  return scan_output;
}

void ScanSimulator2D::scan(const rgc_simulator::Pose2d & pose, double * scan_data) {
...
void ScanSimulator2D::set_map(
    ...
    size_t width_, 
    double resolution_,
    const rgc_simulator::Pose2d & origin_,
