...
class ScanSimulator2D {
  ...
  public:
    ...
    void set_map(
        ...
        size_t width, 
        double resolution,
        const rgc_simulator::Pose2d & origin,
        double free_threshold);
    void set_map(const std::vector<double> &map, double free_threshold);

    void scan(const rgc_simulator::Pose2d & pose, double * scan_data);
    const std::vector<double> scan(const rgc_simulator::Pose2d & pose);
